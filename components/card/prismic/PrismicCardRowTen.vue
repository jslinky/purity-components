...
const isTwoRowCard = computed(() => {
  ...
  return _index % 6 === 2 || _index % 6 === 5
})

const isReverseOrder = computed(() => {
  const _index = index + 1
  return _index % 6 === 1 || _index % 6 === 3
})
...
<template>
  <Card
    ...
    :reverse-order="{ column: isTwoRowCard || isReverseOrder, row: isTwoRowCard || isReverseOrder}"
    :picture="{
  ...
  >
    <template #picture>
      <DefaultCardSlotContentPicture v-if="isTwoRowCard" :card="card" />
    </template>
