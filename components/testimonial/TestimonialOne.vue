...
<template>
  <section class="region @container text-center flow">
    ...
    <div class="layout-grid [--column-count:3] [--flow-space:var(--spacing-2xl)] place-items-center">
      <div v-for="(card, index) in Array.from({ length: 3 })" :key="index" class="[--flow-space:1em] max-w-2xl w-full">
        <Card
          :align-items="{
            column: 'center',
          }"
        ...
        >
          ...
          <template #default>
            <div class="flow">
              <div v-if="logoArr.length > 0" v-html="logoArr[0].svg.el" class="*:w-full *:h-full aspect-[16/3] w-48 mx-auto" />
